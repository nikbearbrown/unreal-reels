...
class BearsDoodlesVideo(Scene):
    ...
    def _card_equation(self, c, t):
        fade = 0.4
        label = txt(c.get("label", "").upper(), 24, DIM, "MEDIUM")
        eq = MathTex(c["tex"], color=INK)
        if is_portrait():
            # 9:16 — the label is BIG and stacked clearly ABOVE the whole equation
            # (not scaled down with it and tucked over the numerator). Size each
            # element to the band, then arrange top→bottom and fit to the band.
            b = band()
            eq.scale_to_fit_width(BL.rw(b) * 0.92)
            label.scale_to_fit_width(BL.rw(b) * 0.92)
            grp = VGroup(label, eq).arrange(DOWN, buff=0.85)
            if grp.height > BL.rh(b) * 0.95:
                grp.scale(BL.rh(b) * 0.95 / grp.height)
            grp.move_to(BL.center(b))
        else:
            # 16:9 — unchanged designed layout
            eq.scale(2.4).move_to([0, -0.1, 0])
            if eq.width > 11:
                eq.scale_to_fit_width(11)
            label.move_to([0, 1.4, 0])
        r1 = min(0.7, t * 0.22)
        r2 = min(1.5, t * 0.4)
