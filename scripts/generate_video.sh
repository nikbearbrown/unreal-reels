#!/usr/bin/env bash
# generate_video.sh — per-beat image+audio -> video for a DANCE beat_sheet.json
...
#
# Usage:
#   ./generate_video.sh <reel_folder>                 # all beats
#   ./generate_video.sh <reel_folder> B01             # one beat (test the beat-match first!)
...
set -uo pipefail
FOLDER="${1:?usage: generate_video.sh <reel_folder> [BEAT_ID...]}"; shift || true
...
# ASSEMBLE_ONLY only cuts+muxes clips you already have — no generation, so no
# higgsfield needed (keeps the free "assemble what's there" path key-less).
if [ "$DRY_RUN" != 1 ] && [ "$ASSEMBLE_ONLY" != 1 ]; then
...
if [ "${FINAL:-0}" = 1 ]; then
  ...
  # ffmpeg's concat demuxer resolves entry paths relative to the LIST FILE's own
  # directory, not the CWD — so write ABSOLUTE paths (works for any invocation).
  absout="$(cd "$OUT" && pwd)"
  for bid in $(jq -r '.beats[].beat_id' "$SPEC"); do
    if [ -f "$OUT/$bid.mp4" ]; then echo "file '$absout/$bid.mp4'" >> "$list"; else missing="$missing $bid"; fi
  done
  if [ -n "$missing" ]; then
